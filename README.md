...
1 - check that the app makes the data call.
2 - check that the data request returns the right data.
3 - check that in case the right data is not returned, the correct error object is returned.
4 - display "News are loading" when there is no data to display.
5 - display the feed of news when there are news.
